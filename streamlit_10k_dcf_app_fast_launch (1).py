...
def extract_pdf_text_cached(file_bytes: bytes, max_pages: int = 45) -> str:
    ...
    Uses pypdf when installed. If pypdf is missing, the app shows a friendly
    message instead of crashing with ModuleNotFoundError.
    """
    try:
        ...
        try:
            ...
            raise RuntimeError(
                "PDF support is not installed. Add `pypdf` to requirements.txt, "
                "commit/push it, and reboot the Streamlit app."
            ) from exc
